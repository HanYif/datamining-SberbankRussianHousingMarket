...
test['rel_floor'] = test['floor'] / test['max_floor'].astype(float) # 层高比
test['rel_kitch_sq'] = test['kitch_sq'] / test['full_sq'].astype(float) # 厨房面积比
...
y_predict = np.round(y_predict * 0.97571)  # From macro model, if my calculations correct
...
y_predict = model.predict(dtest)*.962707  # Can get this by taking macro model average
...
y_pred = model.predict(dtest)*.97381  # From macro model, subject to my calculations
